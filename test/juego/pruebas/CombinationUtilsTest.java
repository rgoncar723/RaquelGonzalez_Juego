...
public class CombinationUtilsTest {
	...
	void testIsChinchon() {
		List<Card> manoChinchon = Arrays.asList(new Card(Rank.ACE, Suit.GOLD), new Card(Rank.TWO, Suit.GOLD),
				new Card(Rank.THREE, Suit.GOLD), new Card(Rank.FOUR, Suit.GOLD), new Card(Rank.FIVE, Suit.GOLD),
				new Card(Rank.SIX, Suit.GOLD), new Card(Rank.SEVEN, Suit.GOLD));

		assertTrue(CombinationUtils.isChinchon(manoChinchon), "Debería ser Chinchón");
	}

	@Test
	void testIsSequenceWrongSuits() {

		List<Card> secuenciaFalsa = Arrays.asList(new Card(Rank.ACE, Suit.GOLD), new Card(Rank.TWO, Suit.CUPS),
				new Card(Rank.THREE, Suit.GOLD));

		assertFalse(CombinationUtils.isSequence(secuenciaFalsa),
				"Una secuencia debe ser obligatoriamente del mismo palo");
	}

	@Test
	void testCalculateUncombinedPointsAll() {
		int esperado = 29;
		List<Card> cartas = Arrays.asList(sieteOros, sotaOros, reyOros);

		int resultado = CombinationUtils.calculateUncombinedPointsAll(cartas);

		assertEquals(esperado, resultado, "La suma total de los puntos nominales debe ser 29");
	}
}
